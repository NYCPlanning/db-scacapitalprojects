...
var output = fs.createWriteStream('output_CancelledProjects.csv')
...
extract(filePath, { layout: "layout" }, function (err, pages) {
  ...
  //we only want 358 thru 364 
     for (var i=357; i<364; i++) {
    ...
    var tableText = pageText.match(/Reason([\s\S]*?)C[0-9]{2}/)[1];
